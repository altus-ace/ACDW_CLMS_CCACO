...
BEGIN TRAN
    ...
    FROM               --readings that are compliant
					(       
						...
						FROM [adw].[2020_tvf_Get_ClaimsByValueSet]('Diastolic Less Than 80', 'Diastolic 80–89', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
						INTERSECT
						SELECT DISTINCT SUBSCRIBER_ID, SEQ_CLAIM_ID
						FROM [adw].[2020_tvf_Get_ClaimsByValueSet]('Systolic Less Than 140', '', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
					)	Firsts
    JOIN
					(
                    ...
                    (
                        ...
                        FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Diastolic Less Than 80', 'Diastolic 80–89'
										, 'Diastolic Greater Than/Equal To 90','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
                        UNION
                        SELECT DISTINCT SUBSCRIBER_ID, SEQ_CLAIM_ID, PRIMARY_SVC_DATE
                        FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Systolic Less Than 140', 'Systolic Greater Than/Equal To 140', '',''
										, CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
                    ) b
					) c
						WHERE			c.Ranks = 1
					) Seconds
    ...
    JOIN
					(
						SELECT DISTINCT SEQ_CLAIM_ID
						FROM [adw].[2020_tvf_Get_ClaimsByValueSet]('Outpatient', 'Nonacute Inpatient', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))
					) Thirds 
    ...
    FROM               --readings that are compliant
					(       
						...
						JOIN
						[adw].[2020_tvf_Get_ClaimsByValueSet]('Systolic Less Than 140', '', '','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear))b
						ON	a.SUBSCRIBER_ID = b.SUBSCRIBER_ID
					)	Firsts
    JOIN
					(

...
BEGIN TRAN
	...
	SELECT			SUBSCRIBER_ID, ValueCodeSystem, ValueCode, ValueCodeSvcDate
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Cervical Cytology', '', '','', CONCAT('1/1/',@MeasurementYear-2), CONCAT('12/31/',@MeasurementYear))
	...
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Cervical Cytology', '', '','', CONCAT('1/1/',@MeasurementYear-4), CONCAT('12/31/',@MeasurementYear)) a
	JOIN			[adw].[2020_tvf_Get_ClaimsByValueSet]('HPV Tests', '', '','', CONCAT('1/1/',@MeasurementYear-4), CONCAT('12/31/',@MeasurementYear)) b

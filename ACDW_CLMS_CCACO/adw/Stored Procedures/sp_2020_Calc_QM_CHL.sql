...
-- =============================================
CREATE PROCEDURE [adw].[sp_2020_Calc_QM_CHL] 
...
BEGIN TRAN
	...
	SELECT DISTINCT SUBSCRIBER_ID, '0', '0', '' FROM [adw].[2020_tvf_Get_ActiveMembers] (@DateofMeasurementYear)
	WHERE AGE BETWEEN 16 AND 24
	AND GENDER = 'F'
	-- Calculating Values for Denominator Header TmpTable
	INSERT INTO		@TmpTable2(SUBSCRIBER_ID)
    SELECT DISTINCT SUBSCRIBER_ID
  	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Pregnancy', 'Sexual Activity', 'Pregnancy Tests','', CONCAT('1/1/', @MeasurementYear),CONCAT('12/31/', @MeasurementYear)) a
	UNION
	SELECT DISTINCT	SUBSCRIBER_ID
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Contraceptive Medications', '', '', '',CONCAT('1/1/', @MeasurementYear),CONCAT('12/31/', @MeasurementYear)) b 
	...
	-- Calculating Values for Denominator Detail TmpTable
	...
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Chlamydia Tests','','','',CONCAT('1/1/', @MeasurementYear),CONCAT('12/31/', @MeasurementYear))

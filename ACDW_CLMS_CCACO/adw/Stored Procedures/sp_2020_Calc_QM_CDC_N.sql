...
BEGIN TRAN
	...
	JOIN			(
						...
						, '', @BeginDateOfClaims, @StartDateOfMeasurementYear)) b
	...
	SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('Urine Protein Tests', 'Nephropathy Treatment', 'CKD Stage 4','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
	SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('ESRD', 'Kidney Transplant', 'ACE Inhibitor/ARB Medications','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
    INSERT INTO		@TmpTable1(SUBSCRIBER_ID)
	SELECT DISTINCT a.SUBSCRIBER_ID
    FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('ESRD', 'Kidney Transplant', 'ACE Inhibitor/ARB Medications','', CONCAT('1/1/', @MeasurementYear), CONCAT('12/31/', @MeasurementYear)) a
    INSERT INTO		@TmpTable1 (SUBSCRIBER_ID)
	SELECT DISTINCT a.SUBSCRIBER_ID 
	FROM			[adw].[tvf_2020_Get_ProvSpec](39,'','','','','') a 

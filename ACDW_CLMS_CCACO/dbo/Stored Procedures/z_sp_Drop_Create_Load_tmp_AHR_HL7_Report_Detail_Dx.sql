CREATE PROCEDURE [dbo].[z_sp_Drop_Create_Load_tmp_AHR_HL7_Report_Detail_Dx]
AS
    BEGIN
        SET NOCOUNT ON;
		...
		/*
		**@DateEnd = Todays Date
		**@ReportInput = Range Date for Reporting
		**@DateStart = Previous year from current Date
		*/
		DECLARE @DateEnd DATE = CAST(GETDATE() AS DATE);
		DECLARE @ReportInput DATE = @DateEnd;
		DECLARE @DateStart DATE = DATEADD(YEAR,-1, @ReportInput); -- last year
        ...
        WHERE b.PRIMARY_SVC_DATE = @DateStart
        EXCEPT
        SELECT a.SUBSCRIBER_ID, 
               diagCode, 
               replace(diagCode, '.', '') [Dx_No_Dec]
        FROM adw.Claims_Diags a
             JOIN adw.Claims_Headers b ON a.SEQ_CLAIM_ID = b.SEQ_CLAIM_ID
        WHERE b.PRIMARY_SVC_DATE = @DateEnd;
        ...
        ([SUBSCRIBER_ID], 
         ...
         [SVC_DATE]
               ...
               WHERE n.EntityType = 1
                     AND PRIMARY_SVC_DATE > @DateStart --and ch.SUBSCRIBER_ID = '{9450649116'
